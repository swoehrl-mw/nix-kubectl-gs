...
Example: `github:swoehrl-mw/nix-kubectl-gs/v4.3.1`
...
```json
{
    "packages": [
        "kubectl@latest",
        "github:swoehrl-mw/nix-kubectl-gs/v4.3.1"
    ]
}
```
...
pkgs.mkShellNoCC {
  packages = [
    pkgs.kubectl
    (builtins.getFlake "github:swoehrl-mw/nix-kubectl-gs/v4.3.1").packages.${builtins.currentSystem}.kubectl-gs
  ];
}
```
