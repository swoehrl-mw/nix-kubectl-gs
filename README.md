...
Example: `github:swoehrl-mw/nix-kubectl-gs/v4.3.0`
...
```json
{
    "packages": [
        "kubectl@latest",
        "github:swoehrl-mw/nix-kubectl-gs/v4.3.0"
    ]
}
```
...
pkgs.mkShellNoCC {
  packages = [
    pkgs.kubectl
    (builtins.getFlake "github:swoehrl-mw/nix-kubectl-gs/v4.3.0").packages.${builtins.currentSystem}.kubectl-gs
  ];
}
```
