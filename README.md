...
Example: `github:swoehrl-mw/nix-kubectl-gs/v5.2.0`
...
```json
{
    "packages": [
        "kubectl@latest",
        "github:swoehrl-mw/nix-kubectl-gs/v5.2.0"
    ]
}
```
...
pkgs.mkShellNoCC {
  packages = [
    pkgs.kubectl
    (builtins.getFlake "github:swoehrl-mw/nix-kubectl-gs/v5.2.0").packages.${builtins.currentSystem}.kubectl-gs
  ];
}
```
